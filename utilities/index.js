...
Util.buildInventoryDisplay = async function (info) {
  ...
  if (data) {
    ...
    <div class="car-details">
      ...
      <div class="car-info">
        <h2 id="car-title">${data.inv_year} ${data.inv_make} ${
      data.inv_model
    }</h2>
        <h2 id="car-price">$${formatter.format(data.inv_price)}</h2>
        <p><span class="car-info-label">Mileage:</span> <span id="car-miles">${formatter.format(
          data.inv_miles
        )}</span></p>
        <p><span class="car-info-label">Color:</span> <span id="car-color">${
          data.inv_color
        }</span></p>
        <p><span class="car-info-label">Description:</span> <span id="car-description">${
    ...
    show = '<p class="notice">Sorry, no  found.</p>';
  }
  return show;
};
...
Util.getClassOptions = async function (class_id = "") {
  ...
  if (classifications) {
    ...
    classifications.rows.forEach((classification) => {
      const isSelected =
        classification.classification_id === class_id ? "selected" : "";
