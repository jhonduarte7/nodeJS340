...
invCont.addClassification = async (req, res) => {
  ...
  if (classResult) {
    ...
    req.flash(
      "notice",
      "Sorry, there was a problem adding the new classification."
    );
    let nav = await utilities.getNav();
...
invCont.addNewInventory = async (req, res) => {
  ...
  if (invResult) {
    req.flash("notice", `New inventory, ${inv_make} ${inv_model}, added.`);
    let nav = await utilities.getNav();
    let classificationList = await utilities.getClassOptions((class_id = ""));
    res.render("inventory/management", {
      title: "Inventory Management",
...
invCont.buildEditInventory = async function (req, res, next) {
  ...
  itemData = itemData[0];
  const classificationSelect = await utilities.getClassOptions(
